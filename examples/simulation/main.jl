...
#     test_data = "examples/simulation/Array_example.jld"
...
function run_model_dr(times::Unitful.Time, interval::Unitful.Time, timestep::Unitful.Time; do_plot::Bool=false, do_download::Bool=true, save::Bool=false, data_dir::String = "out/")
    ...
    yaml_config = "examples/simulation/data_config_sim.yaml"
    view_sql = "examples/simulation/simulation_views.sql"
    db = DataPipeline.fetch_data_per_yaml(yaml_config, data_dir, use_sql=true, sql_file=view_sql, verbose=false)

    ## 1) PREV. LINE 17: scottish population
    # - "human/demographics/population/scotland" : "grid1km/age/persons"
    # - nb. view defined by examples/simulation/simulation_views.sql
    ...
    symptom_pr = DataPipeline.read_estimate(db, "human/infection/SARS-CoV-2/", "symptom-probability", data_type=Float64)
    ...
    param_tab = DataPipeline.read_table(db, "prob_hosp_and_cfr/data_for_scotland", "cfr_byage")
    ...
    # Time exposed
    T_lat = days(DataPipeline.read_estimate(db, sars_cov2_search, "latent-period", data_type=Float64)[1]Unitful.hr)
    # Time asymptomatic
    T_asym = days(DataPipeline.read_estimate(db, sars_cov2_search, "asymptomatic-period", data_type=Float64)[1]Unitful.hr)
    @show T_asym
    # Time pre-symptomatic
    T_presym = 1.5days
    # Time symptomatic
    T_sym = days(DataPipeline.read_estimate(db, sars_cov2_search, "infectious-duration", data_type=Float64)[1]Unitful.hr) - T_presym
    # Time in hospital
    T_hosp = DataPipeline.read_estimate(db, "fixed-parameters/", "T_hos", data_type=Float64)[1]days
    # Time to recovery if symptomatic
    T_rec = DataPipeline.read_estimate(db, "fixed-parameters/", "T_rec", data_type=Float64)[1]days
