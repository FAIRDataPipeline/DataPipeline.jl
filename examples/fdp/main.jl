...
function pre_example()
    ...
    DataPipeline.write_estimate(handle, 0.003,
                                "example/estimate/contact-parameter",
                                "contact-parameter")
    DataPipeline.write_estimate(handle, 5.0,
                                "example/estimate/asymptomatic-period",
                                "asymptomatic-period")
    DataPipeline.write_estimate(handle, 8.0, "example/estimate/recovery-period",
                                "recovery-period")
    DataPipeline.write_estimate(handle, 0.0004,
                                "example/estimate/mortality-rate",
                                "mortality-rate")
    ## finalise code run
    return DataPipeline.finalise(handle)
end
...
function get_model()
    ...
    MORTALITY = 4
    # - rate function
    function seird_rf(output, parameters::Array{Float64, 1},
                      population::Array{Int64, 1})
        output[1] = parameters[CONTACT] * population[SUSCEPTIBLE] *
                    population[INFECTIOUS]
        output[2] = 1 / parameters[ASYMPT] * population[EXPOSED]
        output[3] = 1 / parameters[RECOVER] * population[INFECTIOUS]
        return output[4] = parameters[MORTALITY] * population[INFECTIOUS]
    end
    ...
    function obs_fn!(y::BayesianWorkflows.Observation,
                     population::Array{Int64, 1}, parameters::Array{Float64, 1})
        ...
        return y.val[OBSERVED_STATES] .= [rand(di), rand(dd)]
    end
    # - observation model
    # obs_model = BayesianWorkflows.partial_gaussian_obs_model(2.0; seq = 3)
    function obs_model(y::BayesianWorkflows.Observation,
                       population::Array{Int64, 1}, theta::Array{Float64, 1})
        d = Distributions.Binomial.(population[OBSERVED_STATES],
                                    [PROB_DETECTION_I, PROB_DETECTION_D])
        return Distributions.logpdf(Distributions.Product(d),
                                    y.val[OBSERVED_STATES])
    end
    # - construct model and return
    return BayesianWorkflows.DPOMPModel(MODEL_NAME, N_EVENT_TYPES, seird_rf,
                                        fnic, fnt, obs_model, obs_fn!, T_ZERO)
end
...
function example()
    ...
    handle = DataPipeline.initialise(wc)
    ## read some data:
    beta = DataPipeline.read_estimate(handle,
                                      "example/estimate/contact-parameter",
                                      "contact-parameter")
    asymp_prd = DataPipeline.read_estimate(handle,
                                           "example/estimate/asymptomatic-period",
                                           "asymptomatic-period")
    recov_prd = DataPipeline.read_estimate(handle,
                                           "example/estimate/recovery-period",
                                           "recovery-period")
    mortality = DataPipeline.read_estimate(handle,
                                           "example/estimate/mortality-rate",
                                           "mortality-rate")
    ## run simulation:
...
function whats_my_example()
    ...
    return DataPipeline.whats_my_file(some_filepath)
