...
function initialise(config_file::String, submission_script::String)
    ...
    storage_root_uri = DataPipeline.postentry("storage_root", storage_root_query)
   
    # Register config file
    config_hash = getfilehash(config_file)
    ...
    script_hash = getfilehash(submission_script)
    ...
    repo_root_uri = postentry("storage_root", repo_root_query)
    ...
    coderun_url = postentry("code_run", body)
...
function link_read!(handle::DataRegistryHandle, data_product::String)
    ...
    if isnothing(dp_entry)
        ...
        # Get component url 
        object_entry = getentry(URIs.URI(obj_url))
